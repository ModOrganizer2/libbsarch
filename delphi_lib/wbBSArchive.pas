  ...
  try
    case fType of
      ...
      baTES4, baFO3, baSSE: begin
        ...
        if Assigned(Sync) and fCompress and (aSize >= 32) then begin
          Sync.EndWrite;
          try
        ...
        if Assigned(MemoryStream) then begin
          ...
          wasCompressed := True;
        end else if fCompress and (aSize >= 32) then begin
          msData := TPreallocatedMemoryStream.Create(@aData[0], aSize);
          try
      ...
      baFO4: begin
        ...
        if fCompress and (aSize >= 32) then begin
          msData := TPreallocatedMemoryStream.Create(@aData[0], aSize);
          try
            ZCompressStream(msData, fStream);
...
// Modified: Version for use in non-Borland C/C++
procedure TwbBSArchive.IterateFilesCompat(aProc: TBSFileIterationProcCompat; aContext: Pointer = nil);
var
  i, j: Integer;
begin
  ...
  if Assigned(Sync) then
    case fType of
      baTES3:
        TParallel.&For(Low(fFilesTES3), High(fFilesTES3), procedure(i: Integer; LoopState: TParallel.TLoopState) begin
          if aProc(Self, PChar(fFilesTES3[i].Name), @fFilesTES3[i], nil, aContext) then
            LoopState.Stop;
        end);
      baTES4, baFO3, baSSE:
        TParallel.&For(Low(fFoldersTES4), High(fFoldersTES4), procedure(i: Integer; OuterLoopState: TParallel.TLoopState) begin
          TParallel.&For(Low(fFoldersTES4[i].Files), High(fFoldersTES4[i].Files), procedure(j: Integer; InnerLoopState: TParallel.TLoopState) begin
            if aProc(Self, PChar(fFoldersTES4[i].Name + '\' + fFoldersTES4[i].Files[j].Name), @fFoldersTES4[i].Files[j], @fFoldersTES4[i], aContext) then begin
              OuterLoopState.Stop;
              InnerLoopState.Stop;
            end;
          end);
      ...
      baFO4, baFO4dds:
        TParallel.&For(Low(fFilesFO4), High(fFilesFO4), procedure(i: Integer; LoopState: TParallel.TLoopState) begin
          if aProc(Self, PChar(fFilesFO4[i].Name), @fFilesFO4[i], nil, aContext) then
            LoopState.Stop;
        end);
    end
  else
    case fType of
      ...
      baTES4, baFO3, baSSE:
        for i := Low(fFoldersTES4) to High(fFoldersTES4) do
          for j := Low(fFoldersTES4[i].Files) to High(fFoldersTES4[i].Files) do
            if aProc(Self, PChar(fFoldersTES4[i].Name + '\' + fFoldersTES4[i].Files[j].Name), @fFoldersTES4[i].Files[j], @fFoldersTES4[i], aContext) then
              Break;
      baFO4, baFO4dds:
        for i := Low(fFilesFO4) to High(fFilesFO4) do
          if aProc(Self, PChar(fFilesFO4[i].Name), @fFilesFO4[i], nil, aContext) then
...
procedure TwbBSArchive.ResourceListCompat(const aEntryResultList: TwbBSEntryList; aFolder: string = '');
var
  Folder : string;
  i, j   : Integer;
begin
...
procedure TwbBSArchive.ResolveHashCompat(const aHash: UInt64; const aEntryResultList: TwbBSEntryList);
var
  Len  : Integer;
  i, j : Integer;
begin
